...
@section('content')
    <x-common.page-breadcrumb :pageTitle="'Chat & Komunikasi'" :title="'Chat'" :subtitle="'Hubungi Admin / Staff / Super Admin / Penghuni'" />
    ...
    <div class="flex h-[calc(100vh-200px)] overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-white/[0.03] shadow-sm"
         x-data="{
            ...
            async fetchMessages() {
                ...
                const response = await fetch(`/owner/chat/fetch/${this.selectedUserId}`);
        ...
        <div class="w-1/3 border-r border-slate-100 dark:border-slate-800 flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar">
                @foreach($contacts as $user)
                    <a href="{{ route('owner.chat', ['user_id' => $user->id]) }}" class="flex items-center gap-3 p-4 hover:bg-slate-50 dark:hover:bg-white/5 border-b border-slate-100 dark:border-slate-800">

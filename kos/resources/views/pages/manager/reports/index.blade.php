...
@section('content')
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div class="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-white/[0.03] shadow-sm">
            ...
            <a href="{{ route('manager.reports.export') }}" class="inline-block mt-4 px-4 py-2 rounded-lg bg-primary text-white text-sm font-bold">Download CSV</a>
        </div>
    ...
    <div class="mt-6 rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-white/[0.03] shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50">
            <h3 class="text-lg font-bold text-slate-800 dark:text-white">Daftar Tunggakan</h3>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-100 dark:divide-slate-800">
                <thead class="bg-slate-50 dark:bg-slate-900/50">
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                    @foreach($arrears as $p)
                    <tr>
                        <td class="px-6 py-4">{{ $p->tenant?->name }}</td>
                        <td class="px-6 py-4">{{ $p->room?->number }}</td>
                        <td class="px-6 py-4">Rp {{ number_format($p->amount,0,',','.') }}</td>
                        <td class="px-6 py-4">{{ \Illuminate\Support\Carbon::parse($p->due_date)->format('d/m/Y') }}</td>
                    </tr>
                    @endforeach
                    @if($arrears->isEmpty())
                    <tr><td class="px-6 py-6 text-center text-slate-500" colspan="4">Tidak ada tunggakan.</td></tr>
                    @endif
                </tbody>
            </table>
        </div>

...
@section('content')
    <x-common.page-breadcrumb :pageTitle="'Chat & Komunikasi'" :title="'Chat'" :subtitle="'Hubungi Admin, Super Admin, atau Staff'" />
    ...
    <div class="flex h-[calc(100vh-200px)] overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-white/[0.03] shadow-sm"
         x-data="{
            ...
            async sendMessage() {
                ...
                const response = await fetch('{{ route('manager.chat.send') }}', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
                    body: JSON.stringify({ receiver_id: this.selectedUserId, message: this.newMessage })
                });
                if (response.ok) {
                    this.messages.push({ sender_id: {{ auth()->id() }}, message: this.newMessage, created_at: new Date().toISOString() });
                    this.newMessage = '';
                }
            },
            async fetchMessages() {
                if (!this.selectedUserId) return;
                const response = await fetch(`/manager/chat/fetch/${this.selectedUserId}`);
                if (response.ok) { this.messages = await response.json(); }
            },
            scrollToBottom() { const c = this.$refs.messageContainer; c.scrollTop = c.scrollHeight; },
            init() { if (this.selectedUserId) { this.scrollToBottom(); setInterval(() => this.fetchMessages(), 5000); } }
         }">
        <div class="w-1/3 border-r border-slate-100 dark:border-slate-800 flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar">
                @foreach($contacts as $user)
                    <a href="{{ route('manager.chat', ['user_id' => $user->id]) }}" class="flex items-center gap-3 p-4 hover:bg-slate-50 dark:hover:bg-white/5 border-b border-slate-100 dark:border-slate-800">
                        <div class="relative h-10 w-10 rounded-full bg-primary text-white flex items-center justify-center font-bold">
                            {{ substr($user->name, 0, 1) }}
                            @php $c = $unread[$user->id] ?? 0; @endphp
                            @if($c > 0)

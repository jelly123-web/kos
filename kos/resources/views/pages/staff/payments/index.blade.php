...
@section('content')
    <x-common.page-breadcrumb :pageTitle="'Bantu Pembayaran'" :title="'Pembayaran'" :subtitle="'Catat pembayaran & informasi tagihan'" />
    ...
    <div class="grid grid-cols-1 gap-6">
                ...
                <table class="min-w-full divide-y divide-slate-100 dark:divide-slate-800">
                    ...
                    <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 rounded-full text-xs font-bold {{ $payment->status === 'paid' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700' }}">{{ $payment->status }}</span>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-white/[0.03] shadow-sm overflow-hidden">
            ...
            <div class="p-5">
                <form method="POST" action="{{ route('staff.payments.store') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    @csrf
                    <div>

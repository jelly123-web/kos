...
@section('content')
    <x-common.page-breadcrumb :pageTitle="'Layanan Penghuni'" :title="'Chat'" :subtitle="'Terima keluhan & bantu kebutuhan'" />
    ...
    <div class="flex h-[calc(100vh-200px)] overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-white/[0.03] shadow-sm"
         x-data="{
            ...
            async sendMessage() {
                ...
                const response = await fetch('{{ route('staff.chat.send') }}', {
                    method: 'POST',
            ...
            async fetchMessages() {
                ...
                const response = await fetch(`/staff/chat/fetch/${this.selectedUserId}`);
        ...
        <div class="w-1/3 border-r border-slate-100 dark:border-slate-800 flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar">
                @foreach($contacts as $user)
                    <a href="{{ route('staff.chat', ['user_id' => $user->id]) }}" class="flex items-center gap-3 p-4 hover:bg-slate-50 dark:hover:bg-white/5 border-b border-slate-100 dark:border-slate-800">

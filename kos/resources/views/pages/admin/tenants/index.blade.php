...
@section('content')
    <x-common.page-breadcrumb :pageTitle="'Manajemen Penghuni'" :title="'Penghuni'" :subtitle="'Kelola data penghuni kos'" />
    ...
    <div class="grid grid-cols-1 gap-6">
                    ...
                    <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 text-right">
                                <form class="inline" method="POST" action="{{ route('admin.tenants.update', $tenant) }}">
                                    @csrf
        ...
        <div class="rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-white/[0.03] shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50">
                <h3 class="text-lg font-bold text-slate-800 dark:text-white">Tambah Penghuni Baru</h3>
            </div>
            <div class="p-5">
                <form method="POST" action="{{ route('admin.tenants.store') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    @csrf
                    <div>
                        <label class="text-xs text-slate-500">Nama</label>
                        <input name="name" class="mt-1 w-full border rounded px-3 py-2" placeholder="Nama" required />
                    </div>
                    <div>
                        <label class="text-xs text-slate-500">Telepon</label>
                        <input name="phone" class="mt-1 w-full border rounded px-3 py-2" placeholder="08xxxx" />
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-4">
                        <button class="px-4 py-2 rounded-lg bg-primary text-white">Simpan Penghuni</button>
                    </div>
                </form>
            </div>
        </div>
